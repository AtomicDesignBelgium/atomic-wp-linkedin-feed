...
Version 0.11.0 · GPL-2.0-or-later
...
- One dynamic **Atomic LinkedIn Posts** block and `[atomic_social_feed]` shortcode querying only local WordPress content (no LinkedIn API calls during rendering/pagination).
...
- dynamic Atomic LinkedIn Posts block and `[atomic_social_feed]` shortcode using one query and renderer.
...
Insert **Atomic LinkedIn Posts** in the block editor, or use:
...
LinkedIn embed mode is documented in [LinkedIn embeds](docs/LINKEDIN-EMBEDS.md), including the “View all news” CTA behavior (configured per block instance).
